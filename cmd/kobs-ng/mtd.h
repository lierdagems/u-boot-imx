 ...
 *                            which the underlying physical block mark appears.
 * @ecc_for_meta:             Flag shows if uses ecc for meta.
 */
...
#define PAGES_PER_STRIDE	(mtd_erasesize(md) / mtd_writesize(md))
...
{
	return md->mtd.erasesize;
}

static inline unsigned int mtd_writesize(struct mtd_data *md)
{
	return md->mtd.writesize;
}

static inline unsigned int mtd_oobsize(struct mtd_data *md)
{
	return md->mtd.oobsize;
}

int mtd_isbad(struct mtd_data *md, loff_t ofs);

int mtd_erase_block(struct mtd_data *md, loff_t ofs);

int mtd_read_page(struct mtd_data *md, loff_t ofs, int ecc);
...
void *mtd_load_boot_structure(struct mtd_data *md, loff_t *ofsp, loff_t end,
